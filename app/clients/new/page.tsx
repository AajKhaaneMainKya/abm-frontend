...
import { XpWindow, XpButton, XpProgress, XpDialog, ErrorNote } from "@/components/xp";
    ...
    mutationFn: () => {
      const payload: CreateClientPayload = {
        ...
        icp: {
          ...
          company_size: {
            min: Number(sizeMin) || undefined,
            max: Number(sizeMax) || undefined,
          },
  ...
  return (
    <XpWindow title="New Client Setup" className="mx-auto max-w-2xl">
        ...
        {step === 2 && (
          <>
            ...
            <Field label="Proof points" hint="Comma-separated.">
              <input className={inputCls} value={proofPoints} onChange={(e) => setProofPoints(e.target.value)} placeholder="3x reply rates for Acme, 50 accounts in 24hrs" />
            </Field>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        {!last ? (
          ...
          <XpButton variant="green" disabled={mutation.isPending || !name.trim()} onClick={() => mutation.mutate()}>
            <span className="inline-flex items-center gap-1">
              <Check size={14} /> {mutation.isPending ? "Creating…" : "Create Client"}
            </span>
          </XpButton>
        )}
      </div>
      ...
      <XpDialog
        ...
        footer={
          <>
            <Link href={`/clients/${mutation.data?.id}`}>
              <XpButton variant="primary">Open workspace</XpButton>
            </Link>
            <XpButton onClick={() => router.push("/")}>Dashboard</XpButton>
          </>
        }
      >
